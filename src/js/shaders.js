...
export const qubitVertexShader = `
    ...
    void main() {
        ...
        vLocalPosition = pos; // Pass local position to fragment
    ...
    void main() {
        ...
        if (uPlasmaEnabled > 0.5) {
            ...
            if (uUse3DPlasma > 0.5) {
                ...
                float distortionNoise = snoise(p * 4.0 + uTime * 0.3);
                p.x += distortionNoise * sin(uTime * 0.7) * 0.15;
                p.y += distortionNoise * cos(uTime * 0.7) * 0.15;
                p.z += distortionNoise * sin(uTime * 1.3) * 0.15;
                p = normalize(p);

                float noiseScale1 = 3.0;
                float noiseScale2 = 5.0;
                float noise1 = snoise(p * noiseScale1 + uTime * 0.3);
                float noise2 = snoise(p * noiseScale2 - uTime * 0.2);
                float phi = atan(p.z, p.x); 
                float plasma = sin(phi * 3.0 + noise1 * 3.0 + uTime) * 
                               cos(p.y * 10.0 + noise2 * 3.0 + uTime);
                plasma = (plasma + 1.0) * 0.5;
                plasma = pow(plasma, 1.5);
...
// [NEW] Pure Uniform Volumetric Corona (No Beams, No Noise)
    void main() {
        // Center UVs
        vec2 uv = vUv - 0.5;
        ...
        // --- Pure Light Physics ---
        // Simulating a perfect point light source behind the sphere.
        // using 1/r^2 falloff for realistic light propagation.
        
        // Intensity calculation
        // Reduced constant for tighter core
        float brightness = 0.015 / (dist * dist + 0.002);
        
        // Soft Circular Edge (Vignette)
        // Significantly reduced range for a smaller radius
        // 1.0 - smoothstep ensures compatibility and clear fading
        float vignette = 1.0 - smoothstep(0.05, 0.25, dist);
        
        float glow = brightness * vignette;

        // --- Gentle Pulse ---
        // Minimal brightness variation to keep it feeling "active" but not distracting
        float pulse = 1.0 + 0.05 * sin(uTime * 1.0);
        glow *= pulse;

        // --- Color Mapping ---
        // Core is pure white (saturation loss due to intensity)
        // Middle is the defined color
        // Edge fades out
        
        vec3 coreColor = vec3(1.0, 1.0, 1.0);
        vec3 lightColor = mix(uColor, coreColor, clamp(glow * 0.3, 0.0, 1.0));
        
        // Final Output
        gl_FragColor = vec4(lightColor, glow * uOpacity);
    }
`;
